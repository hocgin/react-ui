...
export default class Toolbar extends React.PureComponent<ToolbarProps, ToolbarState> {
  ...
  render() {
    const { children, batchMenus = [], selectedRows = [], className, style, onClickMenu } = this.props;
    return (
      <div className={classnames(styles.tableListOperator, className)} style={style}>
        {children}
        {batchMenus.length > 0 && (
          <Dropdown disabled={selectedRows.length === 0} overlay={<Menu onClick={onClickMenu}>{batchMenus}</Menu>}>
