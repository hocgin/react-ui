...
import { Tree, Button, Dropdown, Space, Tooltip, Card, Alert } from 'antd';
...
import { handleSchemeColumns, SchemeColumns } from '@/Promise/components/Schema/scheme';
...
const SearchBar: React.FC<{
  ...
  return <Card>
    <ProProvider.Provider value={value}>
      <BetaSchemaForm layoutType={'QueryFilter'} columns={handleSchemeColumns(columns)} />
...
const Index: React.FC<{
  ...
  let { tableAlertOptionRender, title, toolBarRender, useAction, columns = [] } = config;
  ...
  let tree = useRequest(service, { manual: true, onSuccess: (data: TreeNode[]) => setData(data) });
  useMount(() => {
  ...
  return <Space direction={'vertical'} className={styles.page}>
    <Card>
      <ProProvider.Provider value={value}>
        <BetaSchemaForm layoutType={'QueryFilter'} columns={handleSchemeColumns(columns)}
    ...
    <Card>
      <div className={classnames(styles.toolbar, styles.toolbarExt)}>
        ...
        <Space size={16}>
          <Space>
            {toolBarRender && toolBarRender({ reload: () => tree.runAsync({ ...params }) })}
          </Space>
          <Space size={12}>
            <Tooltip title={'刷新'}><ReloadOutlined onClick={() => tree.runAsync({ ...params })} /></Tooltip>
          </Space>
        </Space>
      </div>
      {(selectedRowKeys.length > 0 && tableAlertOptionRender) &&
      <Alert className={styles.alert} message={<div className={styles.tableAlertOption}>
        {option ? <div>{option}</div> : null}
      </div>} />}
      <Tree onSelect={(rows, target) => {
