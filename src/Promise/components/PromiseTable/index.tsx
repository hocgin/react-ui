...
class Index extends PureComponent<PromiseTableProps, PromiseTableState> {
  ...
  render() {
    ...
    return (<ComplexTable title={title} tableLoading={pagingLoading}
                          batchMenus={batchMenus} buttons={buttons} searchItems={searchItems}
  ...
  paging = () => {
    ...
    this.setState({ pagingLoading: true }, () => {
      Utils.POST(`${Config.get(Config.ConfigKeys.baseServerUrl)}${action}/_paging`, { ...searchValue }).then((result: HttpResult<any>) => {
  ...
  onClickOperateRows = (menuInfo: any) => {
    ...
    let returnFlag = onClickOperateRows && onClickOperateRows(menuInfo, selectedRows);
    if (returnFlag === undefined || !returnFlag) {
      return;
    }
    console.debug('-> 进行默认操作');
  ...
  onChangeStandardTable = ({ pageSize, current }: any, filtersArg: any, sorter: any) => {
    let { searchValue } = this.state;
    this.setState({
      searchValue: {
        ...searchValue,
        size: pageSize, page: current,
      },
    }, this.paging);
  };
  ...
  onChangeSelectRow = (rows: any) => {
    ...
    this.setState({ selectedRows: selectedRows }, () => onSelectRow && onSelectRow(selectedRows));
  };
    ...
    return [...tableColumns, {
      ...
      render: (text: string, record: TableData) => {
        const onClickOperateRow = (record: TableData, event: EventInfo) => {
          this.setState({ operateRow: record?.id }, () => this.onClickOperateRow(event, record));
        };
        const RowMenus = (<Menu onClick={onClickOperateRow.bind(this, record)}>
          {rowMenus}
        </Menu>);
