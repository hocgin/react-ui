...
export const HeartFilled: React.FC<{
  ...
  style?: any;
  href?: string;
}> = ({ className, style, href = 'https://www.hocgin.top' }) => {
  return (
    <a className={classnames(styles.a, className)} href={href} target='_blank'>
      <div className={classnames(styles.heart)} style={style}>
        <svg
          width='16px'
          height='16px'
          role='img'
...
const Index: React.FC<{
  href?: string;
  text?: boolean;
}> = ({ href = 'https://hocg.in', text = 'HOCGIN' }) => {
  return (
